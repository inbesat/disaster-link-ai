...
import com.safesphere.nativeapp.data.repository.RoadClosureRepository;
...
public class PublicMapFragment extends Fragment {
    ...
    @Nullable @Override public View onCreateView(@NonNull LayoutInflater inf, @Nullable ViewGroup c, @Nullable Bundle b) {
        return inf.inflate(R.layout.fragment_public_map, c, false);
    }

    @Override public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        super.onViewCreated(view, savedInstanceState);
        mapView = view.findViewById(R.id.publicMapView);
        mapView.onCreate(savedInstanceState);

        ShelterRepository shelterRepo = new ShelterRepository(requireActivity().getApplication());
        RoadClosureRepository closureRepo = new RoadClosureRepository(requireActivity().getApplication());

        mapView.getMapAsync(map -> {
            SafeSphereMapHelper.loadDarkStyle(map, style -> onStyleReady(map, style, shelterRepo, closureRepo));
    ...
    private void onStyleReady(MapLibreMap map, Style style,
                              ...
                              RoadClosureRepository closureRepo) {
        ...
        closureRepo.getActiveRoadClosures().observe(getViewLifecycleOwner(), closures -> {
            if (closures != null) SafeSphereMapHelper.updateClosures(style, closures);
        });
    }
