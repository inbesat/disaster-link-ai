...
export function TimeSliderControl({ hour, onChange }: TimeSliderControlProps) {
  ...
  return (
    <div className="absolute bottom-4 left-1/2 z-30 w-[min(92vw,38rem)] -translate-x-1/2 rounded-xl border border-white/10 bg-[#111827]/90 px-5 py-3.5 shadow-2xl backdrop-blur-md">
      {/* Header row */}
      <div className="mb-3 flex items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          <Brain aria-hidden="true" className="h-4 w-4 text-blue-400" />
          <p className="text-[0.6875rem] font-bold uppercase tracking-[0.14em] text-slate-400">
        ...
        <div className="flex items-center gap-2.5">
          ...
          <button
            ...
            className="flex h-9 w-9 items-center justify-center rounded-lg border border-white/10 bg-white/5 text-white transition hover:scale-105 hover:bg-white/10 active:scale-95"
          >
            {playing ? (
              <Pause aria-hidden="true" className="h-4 w-4" />
            ) : atEnd ? (
              <RotateCcw aria-hidden="true" className="h-4 w-4 text-blue-400" />
            ) : (
              <Play aria-hidden="true" className="h-4 w-4 text-blue-400" />
            )}
          </button>
        ...
        {FORECAST_TICKS.map((t) => (
          <span
            ...
            className={`flex h-4 flex-col items-center gap-0.5 text-[0.5625rem] font-medium ${
              t === hour ? "text-blue-400" : "text-slate-500"
            }`}
          >
      ...
      <div className="mt-1.5 flex items-center justify-between text-[0.625rem] font-medium text-slate-500">
