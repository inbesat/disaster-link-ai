...
import { Cpu, KeyRound, Loader2, PlugZap, ShieldCheck, Sparkles } from "lucide-react";
...
export default function ModelPreferencesCard() {
  ...
  const activeProvider =
    AI_PROVIDERS.find((option) => option.value === settings.provider) ??
    AI_PROVIDERS[0];
  ...
  return (
    ...
    >
      ...
      <p className="mt-3 text-sm text-slate-400">
        Choose which language model powers your AI Command Assistant. Cloud
        providers route through our free-tier endpoints; the local option
        keeps everything fully air-gapped.
      </p>
      ...
      <div className="mt-4">
        ...
        <p className="mt-2 flex items-start gap-2 text-[11px] leading-relaxed text-slate-500">
          ...
          Keys are stored securely in local state and never sent to our
          servers. Leave blank to use the default organizational tier.
        </p>
      </div>
      ...
      <div className="mt-5 flex flex-wrap items-center justify-between gap-3 border-t border-[#1c2740] pt-4">
        <p className="text-[11px] text-slate-500">
          Verifies the configured provider &amp; key are reachable.
        </p>
        <button
          type="button"
      ...
      <div className="mt-4 flex items-start gap-2 rounded-md border border-[#1c2740] bg-surface-muted/40 p-3">
        ...
        <p className="text-[11px] leading-relaxed text-slate-500">
          The assistant defaults to the bundled organizational tier. Adding a
          key overrides the tier for this device only.
        </p>
      </div>
    </section>
  );
